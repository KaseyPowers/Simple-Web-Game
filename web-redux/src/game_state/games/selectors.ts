...
export const getGameSelectors = (id: GameObjIds) => {
  const gameDefStateSelector = createSelector(
    allGamesStates,
    (games) => games[id],
  );
  ...
  const gameStateSelector = createSelector(
    ...
    (gameDef) => gameDef.state,
  );

  const playerIds = createSelector(
    gameDefStateSelector,
    (gameDef) => gameDef.players,
  );

  const playerStates = createSelector(
    gameStateSelector,
    (state) => state.playerStates,
  );

  const gamePlayersSelector = createSelector(
    playerIds,
    selectPlayerProfilesById,
    playerStates,
    (ids: UUID[], profilesById, playerStates) =>
      ids.map((id) => ({ ...profilesById[id], state: playerStates[id] })),
  );
