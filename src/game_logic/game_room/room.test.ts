...
describe("GameRoom class", () => {
  ...
  describe("add and remove players", () => {
    ...
    it("addPlayer should ignore/return false adding a player more than once (duplicate events)", () => {
      const userId = "test_user";
      const secondUser = "test_second_user";
    ...
    it("addPlayer should automatically add an online status to new players", () => {
      const userId = "test_user";
      const secondUser = "test_second_user";
    ...
    it("addPlayer will attempt to set the status as online for existing users", () => {
      // initialize up room with two users, and verify expected data
      const userId = "test_user";
      const secondUser = "test_second_user";
      ...
      // set one user as offline before test and verify
      testRoom["playersOnline"][userId] = false;
      expect(testRoom["playersOnline"]).toEqual({
        [userId]: false,
        [secondUser]: true,
      });
      // re-adding second user should return false for no changes
      expect(testRoom.addPlayer(secondUser)).toBeFalsy();
      // the online status should be unchanged
      expect(testRoom["playersOnline"]).toEqual({
        [userId]: false,
        [secondUser]: true,
      });
      // add new user to make sure it doesn't effect other players status
      // there will be a change for new user so truthy
      expect(testRoom.addPlayer(thirdUser)).toBeTruthy();
      // the existing online status's should be unchanged
      expect(testRoom["playersOnline"]).toEqual({
        [userId]: false,
        [secondUser]: true,
        [thirdUser]: true,
      ...
      // now try re-add offline user
      // there will be a change for offline user changing to online so truthy
      expect(testRoom.addPlayer(userId)).toBeTruthy();
      // the existing online status's should be unchanged
      expect(testRoom["playersOnline"]).toEqual({
        [userId]: true,
        [secondUser]: true,
